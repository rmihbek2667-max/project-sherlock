...
def subscribe_keyboard(lang: str, invite_urls: list[str]) -> InlineKeyboardMarkup:
    rows = []
    for i, url in enumerate(invite_urls, start=1):
        label = t("subscribe_join_button", lang) if len(invite_urls) == 1 else f"{t('subscribe_join_button', lang)} {i}"
        rows.append([InlineKeyboardButton(text=label, url=url)])
    rows.append([InlineKeyboardButton(text=t("subscribe_check_button", lang), callback_data="check_subscription")])
