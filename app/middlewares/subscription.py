...
class SubscriptionGateMiddleware(BaseMiddleware):
    async def _is_subscribed(self, bot: Bot, telegram_id: int) -> bool:
        try:
            ...
            logger.warning("Subscription check failed for %s: %s", telegram_id, exc)
            return True
    ...
    async def __call__(
        ...
        subscribed = await self._is_subscribed(bot, tg_user.id)
        ...
        if event.message:
            await event.message.answer(
                t("subscribe_required", lang), reply_markup=subscribe_keyboard(lang, settings.channel_invite_url)
            )
