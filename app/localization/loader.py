...
def all_translations(key: str) -> set[str]:
    """All localized values for a given key across every loaded language —
    used to match reply-keyboard button presses regardless of the user's
    chosen language, since the button text itself IS the localized string."""
    return {catalog[key] for catalog in _catalog.values() if key in catalog}
