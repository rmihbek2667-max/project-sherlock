...
async def cmd_new_season(message: Message, db_user: User, session) -> None:
    """Usage: /newseason Name | Description | Finale message (optional)
    Creates a season as pure data — deactivates any currently active season.
    The finale message is sent automatically to a player the moment they
    solve the last day's clue (the "who did it and why" reveal)."""
    if not _require_admin(db_user):
        await message.answer(t("admin_not_authorized", db_user.language))
        return
    ...
    if not payload:
        await message.answer("Usage: /newseason Name | Description | Finale message (optional)")
        return
    parts = [p.strip() for p in payload.split("|")]
    name = parts[0]
    description = parts[1] if len(parts) > 1 else ""
    finale_message = parts[2] if len(parts) > 2 else None
...
@router.message(Command("listclues"))
async def cmd_list_clues(message: Message, db_user: User, session) -> None:
    """Usage: /listclues <season_id> — shows each clue's real ID (needed for /releaseclue),
    day number, title, and whether it's released yet."""
    if not _require_admin(db_user):
        await message.answer(t("admin_not_authorized", db_user.language))
        return
    ...
    if not arg.isdigit():
        await message.answer("Usage: /listclues <season_id>")
        return

    result = await session.execute(select(Clue).where(Clue.season_id == int(arg)).order_by(Clue.day_number))
    clues = result.scalars().all()
    if not clues:
        await message.answer("No clues found for that season.")
        return

    lines = []
    for clue in clues:
        status = "released" if clue.release_at else "not released"
        lines.append(f"id={clue.id} | day {clue.day_number} | {clue.title} | {status}")
    await message.answer("\n".join(lines))
